    ...
    return (
        <>
            {recipes.filter(recipe => recipe.id.toString() === currentRecipe.id).map((recipe) => (
                <div key={recipe.id} className="flex-col mx-10 mb-20">
                    ...
                    <div className="bg-stone-200 p-4 rounded-md md:mx-10 my-4">
                        ...
                        <ul>
                            {recipe.ingredients.map((ingredient, index) => (
                                <li key={ingredient.id} className=" has-checked:text-stone-400 ml-3.5 md:text-2xl py-2"><input type="checkbox" className="md:size-5"></input> <label>{ingredient.quantity} {ingredient.unit_of_measure} {ingredient.name}</label></li>
                            ))}
                        </ul>
                    </div>
                    <div className="grid grid-cols-2 md:mx-10">
                        ...
                        <Link className="justify-self-end" to={`/recipe/walkthrough/${recipe.id}`}>
