...
const MealPlanningCard = ({ recipe_id, recipeName, calories, servings, prep_time, cook_time, ingredients, onClick }) => {
    ...
    return (
        <div id={`recipe-${recipe_id}`} className="grid gap-10 md:grid-cols-2 xl:grid-cols-3 bg-stone-200 rounded-md">
            ...
            <ul className="xl:col-span-2">
                {ingredients.map((ingredient, index) => (
                    <li key={index} className="has-checked:text-stone-400 m-5 text-xl xl:text-2xl list-none"><input className="md:size-4 mr-5" type="checkbox" /><label> {ingredient.quantity} {ingredient.unit_of_measure} {ingredient.ingredient}</label></li>
                ))}
            </ul>
        </div>
    )
